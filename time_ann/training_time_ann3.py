...
def collect_mal(mal_path_list,mal_path):
    ...
    mal_size = mal_list.__len__()
    for x in mal_list[:mal_size]:
...
def make_path(start_day,end_day):     # start_day end_day : int
    ...
    for d_day in date_list:
        ...
        if d_day_int >= start_day and d_day_int <= end_day:
            ...
            sizehalf = (int)(mal_list.__len__()/MAL_SIZE)
            mal_list1 = mal_list[:sizehalf]
            #mal_list2 = mal_list[mal_list.__len__()/MAL_SIZE:]
            save_model_name = BASE_PATH+str(date_list[date_list.index(d_day)])+"."+str(date_list[date_list.index(d_day)+MONITER_SIZE-1]+"div"+str(MAL_SIZE))
            #save_model_name2 = BASE_PATH + str(date_list[date_list.index(d_day)]) + "." + str(date_list[date_list.index(d_day) + MONITER_SIZE] + "_1")
            print("start :" + d_day)
            print(save_model_name)
            trainANN(benign_list,mal_list1,save_model_name)
            #print(save_model_name2)
            #trainANN(benign_list, mal_list2, save_model_name2)
